...
public class DynamicClientRegistrationEndpoint {
	...
	public String registerNewClient(@RequestBody String jsonString, Model m, HttpServletResponse response) {
		
		logger.info("this is a register test");
		...
		if (newClient != null) {
			...
			newClient.setClientId(newClient.getClientId());
//			newClient.setClientSecret(null);
//			DefaultApprovedSiteService service = new DefaultApprovedSiteService();
...
//			timeout = cal.getTime();
//		
//			service.createApprovedSite(newClient.getClientId(), "12450.ASDFJWFA", timeout, scope);
			...
			// now save it
			try {
//				Set<String> set = newClient.getGrantTypes();
//				Iterator i = set.iterator();//先迭代出来
//				
//				while(i.hasNext()){//遍历
//					logger.info("grant_type:" + i.next().toString());
//				}
//				
